...
Implementation check (2026-04-28): the existing connector runtime has stable curated provider tool IDs, read-only v1 catalog policies, persisted connector tool-call metadata, normalized missing/expired provider errors, and artifact-refresh audit persistence. Stable connected account labels remain only partially guaranteed because Composio labels can fall back to mutable provider profile fields such as email or name. Therefore the first Live Artifacts increment is scoped to static persisted artifact creation/list/detail/update/pin/archive surfaces; connector-backed refresh execution, refresh UI, refresh tests, and refresh manual QA stay behind the refresh phase gate until stable account labeling is complete.
...
- **Render document**: the user-facing artifact page. The preferred implementation is a sanitized HTML document with bounded JSON data embedded/bound into it; legacy typed JSON tiles remain as a compatibility fallback. Do not execute arbitrary artifact-authored JavaScript.
...
`renderJson` is the legacy tile UI boundary and must not be `unknown` in implementation. New artifacts should prefer `html_page_v1`: sanitized HTML plus bounded `dataJson`, rendered in a sandboxed iframe with app-owned data binding. Define zod schemas for both document and tile shapes in `apps/controller/src/live-artifacts/schema.ts` and reuse equivalent TypeScript types in the web UI.
...
5. Before creating the artifact, call `create_live_artifact`; this tool auto-executes by default to keep chat creation fluid. It must still validate and persist only sanitized HTML/JSON and refreshable read-only sources.
...
- Future manual refresh is granted only for included sources that are explicitly classified as read-only and stored with `refreshPermission: "manual_refresh_granted_for_read_only"`.
...
- Tool tests for `create_live_artifact` auto-execution and sanitization.
- Refresh tests for:
  - no source tiles
